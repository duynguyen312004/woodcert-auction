...
Bảng~\ref{tab:similar-auction-systems} đối chiếu phạm vi, điểm tham khảo và giới hạn của ba nền tảng so với WoodCert Auction.
...
Bảng~\ref{tab:actor-main-functions} tóm tắt trách nhiệm chính của từng tác nhân và hệ thống hỗ trợ.
...
\begin{figure}[htbp]
    ...
    \caption*{\textit{Nguồn: Tổng hợp từ phạm vi nghiệp vụ của hệ thống.}}
\end{figure}
\FloatBarrier
...
Phần này lựa chọn sáu use case đại diện cho chuỗi nghiệp vụ cốt lõi. Đặc tả tương ứng được trình bày tại Bảng~\ref{tab:uc-submit-appraisal}, Bảng~\ref{tab:uc-create-auction}, Bảng~\ref{tab:uc-register-auction}, Bảng~\ref{tab:uc-place-bid}, Bảng~\ref{tab:uc-pay-order-remainder} và Bảng~\ref{tab:uc-open-dispute}. Các chi tiết triển khai như Redis, Lua, MySQL, thứ tự phát sự kiện hoặc cơ chế ghi cố gắng được trình bày ở Chương~\ref{chapter:Experiment}--\ref{chapter:SolutionAndContribution}. Use case ``Thẩm định và lập báo cáo'' cùng các đặc tả bổ sung được chuyển sang Phụ lục~\ref{chapter:AppendixA}.
...
Bảng~\ref{tab:non-functional-requirements} trình bày các yêu cầu phi chức năng ở mức tiêu chí chất lượng. Báo cáo không sử dụng ngưỡng định lượng chưa được phê duyệt hoặc chưa có đo kiểm; kết quả đáp ứng được đối chiếu tại Chương~\ref{chapter:Experiment} và Chương~\ref{chapter:conclusion}.
...
Chương~\ref{chapter:Related_works} đã trình bày kết quả khảo sát bối cảnh thực tế và đối chiếu ba hệ thống tương tự là eBay, Catawiki và Lạc Việt Auction. Thông qua đó, đồ án đã xác định các tác nhân nghiệp vụ, xây dựng danh sách yêu cầu chức năng và phi chức năng cốt lõi cho hệ thống WoodCert Auction. Để giữ cho nội dung chương tập trung vào các luồng nghiệp vụ chính, use case ``Thẩm định và lập báo cáo'' cùng một số đặc tả phân rã bổ sung đã được chuyển sang Phụ lục~\ref{chapter:AppendixA}. Đây là cơ sở quan trọng cho phần phân tích lựa chọn công nghệ và thiết kế kiến trúc chi tiết ở các chương tiếp theo.
